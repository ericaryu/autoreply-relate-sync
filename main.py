...
SHEET_TAB = "자동회신"
RELATE_CONTACT_LIST_ID = "m9UDr4"  # 반드시 Contact 타입 리스트 ID 여야 함
RELATE_BASE_URL = "https://api.relate.so/v1"

# 열 인덱스 (0-based)
COL_NAME = 2        # C열: From (Name)
...
def _patch_contact(
    h: dict, contact_id: str, email: str, custom_fields: list, org_id: str = "", name: str = ""
) -> None:
    """contact PATCH 업데이트."""
...
def upsert_contact(
    api_key: str, org_id: str, email: str, date_str: str, contact_map: dict, name: str = ""
) -> tuple[str, str]:
    ...
    if existing_id:
        _patch_contact(h, existing_id, email, custom_fields, org_id=org_id, name=name)
    ...
    if r.status_code == 422 and "has already been taken" in r.text:
        ...
        if cid:
            ...
            _patch_contact(h, cid, email, custom_fields, name=name)
            return cid, "updated"
        ...
        if cid:
            contact_map[email.lower()] = cid
            _patch_contact(h, cid, email, custom_fields, org_id=org_id, name=name)
            return cid, "updated"
        ...
        for email in valid_emails:
            ...
            try:
                contact_id, contact_action = upsert_contact(
                    api_key, org_id, email, date_val, contact_map, name_val
                )
